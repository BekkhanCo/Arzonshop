package sample.kassa.returnProduct;
...
public class ReturnProduct {
    ...
    void refreshTable(String date) {
        sql = "SELECT REPLACE(FORMAT(SUM(`check`.summa),2),',','') summa,\n" +
                ...
                "WHERE " + (kassirID.isEmpty() ? "" : "check.user_id=" + kassirID + " and") + " `check`.check='Возврат' and `check`.deleted is null and `check`.sana LIKE '%" + date + "%' GROUP BY `check`.`sana` ORDER BY sanafil DESC";
    ...
    void createTable() {
        ...
        for (Chek tovar : sortedList) {
            ...
            sql = "SELECT `check`.`id`,users.uname fish,user_id, `check`,  REPLACE(FORMAT(summa,2),',','') summa," +
                    ...
                    "  WHERE " + (kassirID.isEmpty() ? "" : "check.user_id=" + kassirID + " and") + " `check`.check='Возврат' and `check`.deleted is null and `check`.sana LIKE '%" + tovar.get("sana") + "%'\n" +
